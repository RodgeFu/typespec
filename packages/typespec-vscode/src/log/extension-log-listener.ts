...
export interface ExtensionLogOptions extends LogOptions {
  ...
  callbackOnNotificationButtonClicked?: () => void;
}
...
export class ExtensionLogListener implements LogListener {
  ...
  Log(log: LogItem) {
    ...
    const { showOutput, showPopup, notificationButtonText, callbackOnNotificationButtonClicked } =
      log.options ?? { showOutput: false, showPopup: false };
    ...
    if (showPopup && popupAction) {
      const buttonText = notificationButtonText ?? VIEW_DETAIL_IN_OUTPUT;
      void popupAction(log.message, buttonText).then((value) => {
        if (value === buttonText) {
          if (callbackOnNotificationButtonClicked) {
            callbackOnNotificationButtonClicked();
          } else {
            this.outputChannel?.show(true /*preserveFocus*/);
          }
        }
