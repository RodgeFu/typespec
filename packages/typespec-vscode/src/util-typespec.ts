...
import { ExecOutput, normalizeSlashInPath } from "./util.js";
...
export async function ensureNpmPackageInstalled(packageName: string, version: string | undefined, directory: string, onPreInstall: ()=>Promise<"install" | "skip"> = async ()=>"install", onPostInstall:(output: ExecOutput)=>Promise<void> = async () => {}){
  ...
  if(util.isPackageInstalled(packageName)){
    ...
    logger.info(`Package ${packageName} installed but version mismatch. Expect ${version}, but got ${packageJson.version}. Will try to npm install it with the given version.`);
  }
  else{
    if(!version){
      if(isCompilerVersionSensitivePackage(packageName)){
        ...
        if(compilerVersion){
          ...
          logger.debug(`Compiler version found as ${compilerVersion}, so will try to install ${packageName}@${version}`);
        }
      }
    }
    logger.info(`Package ${packageName} is not installed. Will try to npm install it with version ${version ?? "N/A"}.`);
  }
  ...
  const toError = (str: string) => {
    str.trim().split("\n").forEach(line => logger.error(line, true));
  };
  ...
  switch(preInstallResult)
  {
    case "install":
      ...
      }).then(async (output: ExecOutput) => {
        await onPostInstall(output);
      }, async (err) => {
        logger.debug(`Failed to install ${fullName}. Please check the previous logs for details`);
...
export function getAllMainTspFiles(): Thenable<vscode.Uri[]>{
  return vscode.workspace.findFiles('**/main.tsp').then((uris) => uris.filter((uri) => uri.scheme === 'file' && !uri.fsPath.includes("node_modules")));
}
...
export async function getMainTspFile(): Promise<string | undefined>{
  ...
  else if(files.length === 1)
  {
    ...
    const selected = await vscode.window.showQuickPick(files.map(f => f.fsPath),
      {
        canPickMany: false,
...
export async function loadEmitterOptions(baseDir: string, packageNameOrPath: string, getKey: (s: string) => string, getValue: (v: string) => string): Promise<Record<string, string> | undefined> {
  ...
  if (optionSchema) {
    Object.entries(optionSchema.properties).forEach(([key, value]) => {
      supported[getKey(key)] = (
        getValue((value as any)?.description ?? "no description")
      );
    });
  }
  return supported;
