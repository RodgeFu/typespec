...
export function createServer(host: ServerHost): Server {
  ...
  async function customCompile(param: {doc: TextDocumentIdentifier, options: CompilerOptions}): Promise<CompileResultSlim> {
    ...
    if(result === undefined){
      return {hasError: true, diagnostics: "Failed to get compiler result, please check the compilation output for details", entryPoint: undefined, options: undefined};
    }
    else{
      return {hasError: result.program.hasError(), diagnostics: diagnostics ?? "", entryPoint: result.entryPoint, options: result.program.compilerOptions};
    }
  }
